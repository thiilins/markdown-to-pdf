...
## Palette Studio & WCAG Checker

### 1. Visualização & Aplicação Real (O Fator "Uau")

- **UI Playground (Preview de Componentes):**
- Em vez de ver apenas quadrados coloridos, crie uma aba onde o usuário vê um "Mini Dashboard" ou
  uma "Landing Page Genérica".
- Ao mudar a cor `primary` ou `accent`, os botões, gráficos e cards dessa interface fictícia mudam
  em tempo real. Isso ajuda a validar se a paleta funciona na prática (ex: "Será que esse amarelo
  fica legível num botão de ação?").

### 2. Acessibilidade Avançada (Além do WCAG Básico)

- **Simulador de Tipografia:**
...
- **Shadcn Theme Previewer (Sua ideia original expandida):**
- Você já vai exportar o CSS. Que tal renderizar **ao vivo** um componente complexo do Shadcn (como
  um `Card` com `Button`, `Badge` e `Input` dentro) usando as variáveis CSS que você acabou de
  calcular?
- Isso valida instantaneamente o seu algoritmo de geração de tema `oklch`.
